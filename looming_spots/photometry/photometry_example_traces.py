import looming_spots.photometry.randomised_contrasts_photometry
import numpy as np
...
from looming_spots.photometry import photometry_habituations
from looming_spots.constants import flatui
...
def plot_pre_post_photometry_lsie(mtg):
    ...
    ax2 = plt.subplot(212)
    for t in mtg.pre_test_trials()[:3]:
        plt.plot(t.delta_f()[:TRACK_LENGTH])
        avg_df.append(t.delta_f()[:TRACK_LENGTH])
    mtg.pre_test_trials()[0].plot_stimulus()
...
def plot_escape_metrics_variable_contrast_experiments(
    ...
    for metric in metrics:
        df = looming_spots.photometry.randomised_contrasts_photometry.get_signal_metric_dataframe_variable_contrasts(
            mtgs, metric
        )
        all_dfs.append(df)
