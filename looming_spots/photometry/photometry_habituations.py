...
#from looming_spots.analyse.randomised_contrasts_photometry import get_signal_metric_dataframe_variable_contrasts
...
def plot_max_integral_bars(mtgs):
    ...
    for c, mtg in zip(colors, mtgs):
        ...
        plt.plot(
            [1, 2],
            [np.mean(max_integrals_pre), np.mean(max_integrals_post)],
            color="k",
        )
...
def plot_integral_at_latency_bars(mtgs, bar_colors=("k", "k")):
    ...
    for mtg in mtgs:
        ...
        plt.scatter(
            2, np.mean(max_integrals_post), color="k", zorder=1000, s=60
        )
        plt.plot(
            [1, 2],
            [np.mean(max_integrals_pre), np.mean(max_integrals_post)],
        ...
        plt.xticks(
            [1, 2], ["pre-LSIE protocol", "post-LSIE protocol"], rotation=90
        )
        group_avg_pre.append(np.mean(max_integrals_pre))
        group_avg_post.append(np.mean(max_integrals_post))
    ...
    plt.errorbar(
        1,
        np.mean(group_avg_pre),
        scipy.stats.sem(group_avg_pre, axis=0),
        color="Grey",
    )
    plt.errorbar(
        2,
        np.mean(group_avg_post),
        scipy.stats.sem(group_avg_post, axis=0),
        color="Grey",
    )
    print(scipy.stats.ttest_ind(group_avg_pre, group_avg_post))
...
def plot_habituation_curve_with_sem(
    mtgs, color="k", linewidth=3, plot_dots=True
):
...
def get_signal_metric_dataframe(mtgs, metric):
    ...
    for mtg in mtgs:
        norm_factor = max(
            [t.integral_escape_metric() for t in mtg.loom_trials()[:30]]
...
def get_trials_df(mtgs, metric):
    ...
    for mtg in mtgs:
        norm_factor = max(
            [t.integral_escape_metric() for t in mtg.all_trials]
        )  # t.integral_downsampled()[200:214]
        event_metric_dict = {}
        vals = []
        signals = []
