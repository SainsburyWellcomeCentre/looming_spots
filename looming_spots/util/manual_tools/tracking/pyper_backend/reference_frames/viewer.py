...
class Viewer(object):
    ...
    def on_click(self, event, step_size=4):
        if event.button == 1:
            self.frame_idx = min(
                self.frame_idx + step_size, self.video.shape[0] - 1
            )
            self.update()
        if event.button == 3:
    ...
    def on_key_press(self, event):
        if event.key == "left":
            self.left_ref = HalfRef(
                self.ref,
                "left",
                self.current_video_name,
                self.frame_idx,
                self.video[self.frame_idx],
            )
            print("left ref idx: {}".format(self.frame_idx))

        elif event.key == "right":
            self.right_ref = HalfRef(
                self.ref,
                "right",
                self.current_video_name,
                self.frame_idx,
    ...
    def load_video(self):
        ...
        if self.video_path.endswith("mp4"):
            return skvideo.io.vread(
                self.video_path, num_frames=VIEWER_N_FRAMES_LIMIT
            )
    ...
    def make_reference_frame(left_frame, right_frame, mirror_plane_idx):
        ...
        composite_frame[:, :mirror_plane_idx, :] = left_frame[
            :, :mirror_plane_idx, :
        ]
        composite_frame[:, mirror_plane_idx:, :] = right_frame[
            :, mirror_plane_idx:, :
        ]
        return composite_frame

    def save_reference_frame(self):
        reference_frame = self.make_reference_frame(
            self.left_ref.frame,
            self.right_ref.frame,
        ...
        save_fpath = os.path.join(
            self.directory, "{}_ref.png".format(self.namestr)
        )
...
class Ref(object):
    def __init__(self, directory, path=None):
        ...
        self.metadata = looming_spots.db.metadata.experiment_metadata.load_metadata(
            directory
        )
    ...
    def load_from_metadata(self):
        ...
        for item in self.metadata["reference_frame"]:
            if (
                item == "left"
            ):  # TODO: generic concatenation row and column-wise from matrix as list of image pieces
...
class HalfRef(object):
    def __init__(
        self, ref, side=None, video_name=None, frame_idx=None, frame=None
    ):
        self.ref = ref
        self.side = side
    ...
    def absolute_frame_idx(self):
        ...
        abs_idx = (
            int(self.frame_idx) + int(loom_frame_idx) - 200
        )  # TODO: test to ensure the frames are identical
        return abs_idx

    def initialise_metadata(self):
        if (
            self.side not in self.ref.metadata["reference_frame"]
    ...
    @property
    def image(self):
        return looming_spots.util.video_processing.get_frame(
            self.video_name, self.frame_idx
        )
