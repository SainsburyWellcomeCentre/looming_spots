...
def get_mouse_position_at_loom_onset(loom_folder):
    ...
    x_at_loom_onset, y_at_loom_onset = (
        x[CLASSIFICATION_WINDOW_START],
        y[CLASSIFICATION_WINDOW_START],
    )
    return x_at_loom_onset, y_at_loom_onset


def get_track_corrections(session_folder):
    manual_looms_mtd = photodiode.get_manual_looms_from_metadata(
        session_folder
    )
...
def plot_track(
    loom_folder, context, color, zorder=0, smooth=True, alpha=1, label=None
):
    track = load_normalised_track(loom_folder, context)
    if smooth:
        plt.plot(
            gaussian_filter(track, 3),
            color=color,
            zorder=zorder,
            alpha=alpha,
            label=label,
