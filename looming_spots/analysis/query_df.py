...
def get_all_context_combos(df):
    ...
    for habituation_context, post_test_context in all_combos:
        ...
        habituation_df = exclude_pre_tests_df[exclude_pre_tests_df['test_type'] == 'habituation']
        post_test_df = exclude_pre_tests_df[exclude_pre_tests_df['test_type'] == 'post_test']

        habituations_in_context_df = habituation_df[habituation_df.isin([habituation_context])['context']]
        post_tests_in_context_df = post_test_df[post_test_df.isin([post_test_context])['context']]

        mids_in_group = set(habituations_in_context_df['mouse_id']).intersection(post_tests_in_context_df['mouse_id'])

        results_dict.setdefault(''.join([habituation_context, post_test_context]), mids_in_group)
...
def plot_from_mid_dict(condition_mouse_id_dictionary):
    ...
    for j, (condition, mids) in enumerate(condition_mouse_id_dictionary.items()):
        for mid in mids:
