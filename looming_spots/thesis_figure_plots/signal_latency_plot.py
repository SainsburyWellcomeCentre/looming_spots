...
def get_signal_df_mtgs(groups, timepoint=215):
    ...
    for group in groups:
        ...
        for mtg in mtgs:
            ...
            normalising_factor = (
                photometry_example_traces.get_normalisation_factor(
                    mtg, timepoint
                )
            )
    ...
    all_df.to_csv(
        f"/home/slenzi/thesis_latency_plots/signal_df_naive_all_ctst.csv"
    )
    return all_df
...
def plot_mouse_pre_post_tests(
    mtg,
    normalisation_factor,
    normalisation_factor_trace,
    post_test_trials,
    pre_test_trials,
    ...
    for t in pre_test_trials:
        ...
        t.plot_delta_f_with_track(
            norm_factor=normalisation_factor_trace, color=color
        )
        plt.ylim([0, 1])
        [plt.axvline(x, color="k", ls="--") for x in LOOM_ONSETS]
        plt.xlim([180, 370])
...
def calculate_theoretical_escape_threshold(
    mtg, fig=None, axes=None, label=None
):
    (
        normalisation_factor,
        normalisation_factor_trace,
    ...
    for t in post_test_trials:
        ...
        max_val_reached = np.nanmax(
            (t.integral_downsampled() / normalisation_factor)[:335]
        )
        if max_val_reached > theoretical_escape_threshold:
            plot_threshold_and_sub_threshold_trialwise(
                axes[:2],
                mtg,
                normalisation_factor,
                normalisation_factor_trace,
        ...
        fig.savefig(
            f"/home/slenzi/thesis_latency_plots/{fname}.eps", format="eps"
        )
...
def get_all_variables(mtg):
    ...
    pre_test_latency = np.nanmean(
        [t.latency_peak_detect() for t in pre_test_trials]
    )
    pre_test_trial_integral_metric_values = [
        t.integral_escape_metric(int(pre_test_latency))
        for t in pre_test_trials
    ]
    normalisation_factor = np.nanmax(
        [
            t.integral_escape_metric(int(pre_test_latency))
            for t in mtg.loom_trials()
        ]
    )
    normalisation_factor_trace = np.nanmax(
        [np.nanmax(t.delta_f()[200:350]) for t in mtg.loom_trials()]
    )
    theoretical_escape_threshold = (
        np.mean(pre_test_trial_integral_metric_values) / normalisation_factor
    )
    theoretical_escape_threshold_minimum = (
        np.min(pre_test_trial_integral_metric_values) / normalisation_factor
    )
    theoretical_escape_threshold_maximum = (
        np.max(pre_test_trial_integral_metric_values) / normalisation_factor
    )
...
def plot_mouse_trials_separate_scaled(
    label,
    mtg,
    normalisation_factor,
    normalisation_factor_trace,
    ...
    for i, t in enumerate(post_test_trials):
        ...
        [plt.axvline(x, color="k", ls="--") for x in LOOM_ONSETS]
        plt.plot(
            t.integral_downsampled()[: int(t.latency_peak_detect())]
            / normalisation_factor,
            color=color,
        )
        plt.plot(
...
def plot_threshold_and_sub_threshold_trialwise(
    axes,
    mtg,
    normalisation_factor,
    normalisation_factor_trace,
    ...
    plt.axhline(
        theoretical_escape_threshold / theoretical_escape_threshold,
        color=color,
        linewidth=2,
    )
    [plt.axvline(x, color="k", ls="--") for x in LOOM_ONSETS]
    plt.plot(
        t.integral_downsampled()
        / theoretical_escape_threshold
        / normalisation_factor,
        color=color,
    )  # normalisation_factor
...
def plot_all_integrals_normalised_to_threshold(mtgs, label):
    ...
    for mtg in mtgs:
        ...
        for t in pre_test_trials:
            ...
            plt.plot(
                t.get_integral(delta_f_normalised)[: int(latency)], color=color
            )
        ...
        for t in post_test_trials:
            ...
            plt.axis("off")
    fig.savefig(
        f"/home/slenzi/thesis_latency_plots/{fname}_all.eps", format="eps"
    )
...
def plot_optional_metrics(
    pre_test_latency, t, normalisation_factor, max_val_reached=None
):
    latency = t.latency_peak_detect()
    if latency is not None:
        print(f"latency: {latency}")
        if latency < 600:
            plt.axhline(
                t.integral_downsampled()[int(latency)] / normalisation_factor,
                color="b",
                ls="--",
            )
...
def plot_pre_test_trial(mtg, pre_test_trials):
    ...
    for t in pre_test_trials:
        ...
        plt.axis("off")
        fig.savefig(
            f"/home/slenzi/thesis_latency_plots/{title}.eps", format="eps"
        )
        plt.close()
...
def plot_all_theoretical_escape_thresholds():
    for k in LSIE_SNL_KEYS:
        ...
        for mtg in mtgs:
            calculate_theoretical_escape_threshold(
                mtg, fig=fig, axes=axes, label=k
            )
...
def get_df_non_escape_relative_to_estimated_threshold_mtg(mtg):
    ...
    if mtg.mouse_id == "898990":
        escapes = [True] * 3
    theoretical_escape_thresholds = [theoretical_escape_threshold] * len(
        post_test_trials
    )
...
def get_df_non_escape_relative_to_estimated_threshold():
    ...
    df_all["exceeds theoretical threshold"] = (
        df_all["deltaf max in trial"] > df_all["escape ∆F threshold"]
    )
    df_all["exceeds theoretical threshold short"] = (
        df_all["deltaf max in trial up to 5th"] > df_all["escape ∆F threshold"]
    )
...
def replot_lsie():
    ...
    mids_sameday = experimental_log.get_mouse_ids_in_experiment(
        LSIE_SNL_KEYS[1]
    )

    fig = photometry_example_traces.plot_LSIE_bars_all_groups(
        groups=(mids24, mids_sameday)
    )
...
def plot_895773_latency_escapes():
    ...
    t.plot_stimulus()
    fig.savefig(
        "/home/slenzi/thesis_latency_plots/example_avg_latency.eps",
        format="eps",
    )
    ...
    if first_latency is not None:
        ...
        for t, color in zip(pre_test_trials, ["b", "g", "orange"]):
            ...
            if latency is not None:
                ...
                plt.plot(
                    t.integral_downsampled()[: int(latency) + 1], color=color
                )
                plt.plot(
                    t.integral_downsampled()[: int(latency) + 1]
                    * scale_factor,
                    linestyle="dotted",
                    color=color,
                )
                print(
                    f"FIRST: {first_thresh}, TRIAL: {val_at_latency}, SCALE_FACTOR: {scale_factor} NEW MAX: {t.integral_downsampled()[int(latency)]*scale_factor}"
                )
    [plt.axvline(x, color="k", ls="--") for x in LOOM_ONSETS]
...
def plot_all_integrals_to_latency_with_predictions():
    ...
    for mtg in mtgs:
        ...
        [plt.axvline(x, color="k", ls="--") for x in LOOM_ONSETS]
    fname = f"integral_post_test_all_scaled_to_mean"
    fig.savefig(f"/home/slenzi/thesis_latency_plots/{fname}.eps", format="eps")
