...
def get_all_directories(
    mid, root=r"Z:\margrie\glusterfs\imaging\l\loomer\processed_data"
):
...
def get_control_escapes():
    all_mids = []
    keys = [
        "pre_test_control_same_day",
        "pre_test_two_weeks_apart",
        "auditory_white_noise_with_pre_test_-++",
        "auditory_white_noise_with_pre_test_---",
        "auditory_white_noise_with_pre_test_and_delay---+-",
        "naive_escape_control",
        "baseline_flee_rates_in_different_contexts",
        "naive_escape_in_A",
        "molly_mouse_pre_test",
        "pre_test_effect_on_gradient_protocol",
        "control_escape_adapted_arena_photometry",
        "control_3+weeks_isolated_pre_hab_post",
        "spot_contrast_cossell_curve_ct",
        "pre_hab_post_24hr",
...
def plot_lsie_metric_distributions():
    ...
    for metric in metrics:
        ...
        df_lsie = escape_metric_dataframes.get_behaviour_metric_dataframe(
            mtgs_lsie, metric, "post_test"
        )

        mids_control = get_control_escapes()
        mtgs_control = [
            loom_trial_group.MouseLoomTrialGroup(mid) for mid in mids_control
        ]
        df_control = escape_metric_dataframes.get_behaviour_metric_dataframe(
            mtgs_control, metric, "pre_test"
        )
        plt.figure()
...
def lsie_control_escape_plots():
    ...
    mids_spot_var = experimental_log.get_mouse_ids_in_experiment(
    ...
    mtgs_lsie = [
        loom_trial_group.MouseLoomTrialGroup(mid) for mid in mids_lsie
    ]
    df_lsie = escape_metric_dataframes.get_behaviour_metrics_dataframe(
        mtgs_lsie, metrics, "post_test", "LSIE"
    )

    mtgs_ctrl = [loom_trial_group.MouseLoomTrialGroup(mid) for mid in mids]
    df_ctrl = escape_metric_dataframes.get_behaviour_metrics_dataframe(
        mtgs_ctrl, metrics, "pre_test", "CONTROL"
    )
    ...
    plt.figure()
    sns.scatterplot(
        x="time to reach shelter stimulus onset",
        y="speed",
        data=df_ctrl,
        s=50,
        marker="+",
...
def plot_heatmap(
    trials, metric="latency peak detect", stimulus_onset=200, limit=150
):
    ...
    for t in trials:
        speeds.append(
            -t.smoothed_x_speed[stimulus_onset : stimulus_onset + limit]
        )
    ...
    for mtg in mtgs:
        for t in mtg.loom_trials()[:3]:
            ...
            if latency is not None:
                diffs = [
                    x - latency for x in LOOM_ONSETS + [LOOM_ONSETS[-1] + 28]
                ]
