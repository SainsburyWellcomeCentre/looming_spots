...
from looming_spots.thesis_figure_plots.signal_latency_plot import (
    get_all_variables,
)
...
def get_df(mtg, group):
    ...
    first_trial_latency = int(
        first_trial.metric_functions["latency peak detect samples"]()
    )

    expected_integral_at_escape_onset = first_trial.integral_downsampled()[
        first_trial_latency
    ]
    ...
    for t in pre_test_trials + post_test_trials:
        ...
        max_integral_reached_by_end_of_stimulus = np.nanmax(
            t.integral_downsampled()[:340]
        )
        max_integral_reached_by_5th_loom = np.nanmax(
            t.integral_downsampled()[:312]
        )

        max_integrals_by_end_of_stimulus.append(
            max_integral_reached_by_end_of_stimulus
        )
        max_integrals_by_end_of_5th_loom.append(
        ...
        if latency is not None:
            ...
            difference_from_expected.append(
                expected_integral_at_escape_onset - integral_at_latency
            )
            percent_of_expected.append(
                integral_at_latency / expected_integral_at_escape_onset
            )
            print(
            ...
            print(
                f"loomnumber:{t.loom_number}_{mtg.mouse_id}:::::_{expected_integral_at_escape_onset - max_integral_reached_by_end_of_stimulus}"
            )
            difference_from_expected.append(
                expected_integral_at_escape_onset
                - max_integral_reached_by_end_of_stimulus
            )
            percent_of_expected.append(
                max_integral_reached_by_end_of_stimulus
    ...
    theoretical_escape_thresholds = [theoretical_escape_threshold] * len(
        pre_test_trials + post_test_trials
    )

    df_dict.setdefault("escape ∆F threshold", theoretical_escape_thresholds)
    df_dict.setdefault(
        "mouse id", [mtg.mouse_id] * len(pre_test_trials + post_test_trials)
    )
    ...
    df_dict.setdefault(
        "deltaf max in trial up to 5th", max_integrals_by_end_of_5th_loom
    )
    df_dict.setdefault("integral at latency", integral_reached_by_latency)
    df_dict.setdefault(
        "expected integral",
        [expected_integral_at_escape_onset]
        * len(pre_test_trials + post_test_trials),
    )
    ...
    df_dict.setdefault(
        "group", [group] * len(pre_test_trials + post_test_trials)
    )
    df_dict.setdefault(
        "trial type",
        ["pretest"] * len(pre_test_trials)
        + ["posttest"] * len(post_test_trials),
    )
...
def get_df_non_escape_relative_to_estimated_threshold():
    ...
    df_all.to_csv(
        "/home/slenzi/thesis_latency_plots/df_threshold_differences.csv"
    )
