...
def plot_escape_and_delta_f_with_latencies():
    for mid in mouse_ids:
        ...
        pre_latency = int(np.mean([t.estimate_latency(False) for t in mtg.pre_test_trials()[:3]]))
...
def plot_all_metrics_sanity(mid):
    ...
    for t in mtg.loom_trials()[:3]:
        ...
        plt.xlim([0, 600])
        plt.ylim([0, 1])
        for point, c in zip([acc_arg, latency, arg_peak_speed, ttrs, ttls, rt, latency_pd], ['k', 'r', 'b', 'y', 'g', 'c', 'm']):
