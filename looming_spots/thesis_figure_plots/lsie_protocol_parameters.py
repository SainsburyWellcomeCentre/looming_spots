...
def plot_effect_of_delay_on_habituation_efficacy(metric='classified as flee'):
    ...
    for contrast_protocol in [2, 7]:
        for delay in [0, 10]:
            ...
            experiment_string = '== variable_stimulus_number_habituation_{}min_delay'.format(delay)
            gradient_protocol_string = '== {}'.format(contrast_protocol)

            test_contrast_df = experimental_log.filter_df(records_df, {'experiment': experiment_string, 'gradient_protocol': gradient_protocol_string})

            if len(test_contrast_df) > 0:
                test_contrast_mids = test_contrast_df['mouse_id'].unique()
                groups.append(test_contrast_mids)

            gradient_contrast_df = experimental_log.filter_df(records_df, {'experiment': experiment_string, 'gradient_protocol': gradient_protocol_string})
            ...
            for mid_group in groups:
                mtgs = [loom_trial_group.MouseLoomTrialGroup(mid) for mid in mid_group]
...
def n_stimuli_effect(metric='classified as flee'):
    ...
    for contrast_protocol in [14, 15, 16]:
        for delay in [0]:
            ...
            for mid_group in groups:
                print('contrast protocol: {}, mouse ids: {}'.format(contrast_protocol, mid_group))
                mtgs = [loom_trial_group.MouseLoomTrialGroup(mid) for mid in mid_group]
                df = get_behaviour_metric_dataframe(mtgs, metric, 'post_test')
                df['experiment'] = [experiment_string.split(' ')[-1]]*len(df)
                df['contrast_protocol'] = [contrast_protocol]*len(df)
