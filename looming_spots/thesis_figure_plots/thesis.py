...
from looming_spots.thesis_figure_plots import (
    randomised_contrast_escape_curves_lesions,
)
from looming_spots.thesis_figure_plots.randomised_contrast_escape_curves_lesions import (
    get_behaviour_metrics_df,
    get_escape_curve_df,
)
...
def plot_all_metrics(
    mids=("CA452_1",), metrics=("speed", "acceleration", "time in safety zone")
):
    ...
    plt.sca(axes[-1])
    ax = sns.lineplot(
        data=df, x="contrast", y="escape", err_style="bars", color="k"
    )
    ax.invert_xaxis()
...
def make_multivariable_plotting_df(df):
    ...
    for metric in df["metric"].unique():
        ...
        df_multivar["experimental group"] = df[df["metric"] == metric][
            "experimental group"
        ].values
        df_multivar["contrast"] = df[df["metric"] == metric]["contrast"].values
        df_multivar["escape"] = df[df["metric"] == metric]["escape"].values
        df_multivar["loom number"] = df[df["metric"] == metric][
            "loom number"
        ].values
    return df_multivar


def get_main_metric_df_multiple_groups(
    group_keys=("CONTROL", "OHDA"),
    metrics=(
        "speed",
        "acceleration",
        "time in safety zone",
...
def plot_lesion_group_comparison(
    ...
    if not os.path.isfile(str(save_path)) or reload:
        ...
        main_metric_df_all_groups = get_main_metric_df_multiple_groups(
            metrics=(
                ...
                "time to reach shelter detection",
                "reaction time",
            ),
    ...
    main_metric_df_all_groups = main_metric_df_all_groups[
        main_metric_df_all_groups["experimental group"].isin(group_keys)
    ]
    ...
    # plot figures
    plot_all_metrics_against_contrast(
        df, figure_dir, flatui, format_plots, metric_labels, fmt=fmt
    )
    plot_all_metric_distributions(
        df, figure_dir, flatui, metric_labels, fmt=fmt
    )

    first_label = main_metric_df_all_groups["metric"].unique()[0]
    single_metric_df = main_metric_df_all_groups[
        main_metric_df_all_groups["metric"] == first_label
    ]

    plot_test_contrast_trials_in_order(
        figure_dir, flatui, single_metric_df, fmt=fmt
    )
    plot_all_metrics_bars_at_test_contrast(
        figure_dir,
        flatui,
        format_plots,
    ...
    if stats:
        ...
        for combination in itertools.combinations(group_keys, 2):
            compute_stats_mixed_anova(
                combination[0], combination[1], main_df_name=fname
            )
        ...
        for metric in [
            ...
            subdf = main_metric_df_all_groups[
                main_metric_df_all_groups["metric"] == metric
            ]
            ...
            a.to_csv(
                str(Path(save_dir) / f"stats_ttest_test_contrast_{metric}.csv")
            )
...
def plot_all_metrics_bars_at_test_contrast(
    figure_dir,
    flatui,
    format_plots,
    main_metric_df_all_groups,
    ...
    barplot_df = main_metric_df_all_groups[
        main_metric_df_all_groups["metric"].isin(metric_labels)
    ]
    sns.barplot(
        "metric value",
        "metric",
...
def plot_test_contrast_trials_in_order(
    figure_dir, flatui, single_metric_df, fmt="eps"
):
    fig = plt.figure()
    ...
    single_metric_df["loom number"] = single_metric_df["loom number"].astype(
        str
    )
    ...
    for i, group in enumerate(single_metric_df["experimental group"].unique()):
        plt.figure()
        plt.title(group)
        groupdf = single_metric_df[
            single_metric_df["experimental group"] == group
        ]
        sns.swarmplot(
            x="loom number", y="escape", hue="mouse id", data=groupdf
        )
...
def plot_all_metric_distributions(
    df, figure_dir, flatui, metric_labels, xlim_upper=120, fmt="eps"
):
    for metric in metric_labels:
        ...
        for i, group_df_label in enumerate(df["experimental group"].unique()):
            ...
            n_bins = abs(
                (np.nanmax(group_df[metric]) - np.nanmin(group_df[metric])) / 3
            )
...
def plot_all_metrics_against_contrast(
    df, figure_dir, flatui, format_plots, metric_labels, fmt="eps"
):
    for metric in metric_labels:
        fig = plt.figure()
...
def compute_stats_mixed_anova(
    ...
    mixed_anova_result_path = (
        save_dir / f"mixed_anova_{group_label1}_vs_{group_label2}.csv"
    )
    mixed_anova_post_hocs_path = (
        save_dir
        / f"mixed_anova_post_hocs_{group_label1}_vs_{group_label2}.csv"
    )
    ...
    two_groups_df = single_metric_df[
        single_metric_df["experimental group"].isin(
            [group_label1, group_label2]
        )
    ]
    escape_curve_df = (
        randomised_contrast_escape_curves_lesions.get_escape_curve_df(
            two_groups_df
        )
    )
    ...
    print(
        summary_table[
            summary_table["Contrast"] == "contrast * experimental group"
        ].to_latex()
    )
...
def plot_scatter_by_contrast(
    ...
    for metric in metrics:
        ...
        g.savefig(
            str(figure_dir / f"contrast_signal_{metric}_{group_label}.{fmt}"),
            fmt=fmt,
        )
        all_plots.append(g)
    ...
    sns.pointplot(
        "contrast", "escape", color="k", data=df, err_style="bars", scale=2
    )

    g.savefig(
        str(figure_dir / f"contrast_signal_escape_{group_label}.{fmt}"),
        fmt=fmt,
    )
    fig = plt.figure()
    n_bins = 10
    ...
    fig.savefig(
        str(figure_dir / f"escape_displot_signal_{group_label}.{fmt}"), fmt=fmt
    )
...
def get_df_metrics_as_columns(
    ...
    df = escape_metric_dataframes.get_behaviour_metrics_dataframe(
        mtgs, metrics, "variable_contrast", experimental_group_label
    )
    return df
...
def get_track_df(mids):
    mtgs = [loom_trial_group.MouseLoomTrialGroup(mid) for mid in mids]
    df = escape_metric_dataframes.get_track_dataframe(
        mtgs, "variable_contrast"
    )
    return df
