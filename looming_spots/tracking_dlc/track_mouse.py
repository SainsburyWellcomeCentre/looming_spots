...
def save_npy_tracks(directory, config_path_label):
    ...
    if tracks_path is not None:
        ...
        mouse_xy_tracks = process_DLC_output.process_DLC_output(str(tracks_path), config_path_label)
...
def track_mouse(mid, overwrite=False):
    ...
    for vpath in vpaths:
        track_5label.track_5_label(pathlib.Path(vpath), config_path_label='bg_5_label_SL_new', overwrite=overwrite)
