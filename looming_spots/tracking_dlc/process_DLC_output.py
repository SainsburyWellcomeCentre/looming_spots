...
def process_DLC_output(file_path, config_path_label):
    ...
    median_label_tracks = get_median_position_from_labels(clean_data, config_path_label)
...
def replace_low_likelihood_as_nan(extracted_tracks, likelihood_threshold=0.9999):
...
def get_first_and_last_likely_frame(extracted_tracks, body_part_label, n_samples=500):
    body_part = extracted_tracks[body_part_label]
...
def get_median_position_from_labels(dlc_clean_data, config_path_label='one_label_transform'):
    body_part_labels = BODYPART_LABELS[config_path_label]
    body_parts = {body_part_label: dlc_clean_data[body_part_label] for body_part_label in body_part_labels}
