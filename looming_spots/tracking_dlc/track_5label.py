...
from looming_spots.tracking_dlc.track_mouse import get_tracks_path, save_npy_tracks


def track_5_label(vpath, config_path_label='bg_5_label', label_video=False, overwrite=False):
    all_paths_raw = get_tracks_path(vpath.parent, config_path_label, get_all_paths=True)

    d = Path(str(vpath.parent).replace('raw_data', 'processed_data'))
    directory = d / '5_label'
    all_paths = get_tracks_path(directory, config_path_label, get_all_paths=True)
    ...
    if tracks_path is not None:
        ...
        mouse_xy_tracks = process_DLC_output.process_DLC_output(tracks_path, config_path_label)

        mouse_positions_x_path = directory / 'dlc_x_tracks.npy'
        mouse_positions_y_path = directory / 'dlc_y_tracks.npy'
        print(f"saving tracks to {mouse_positions_x_path} and {mouse_positions_y_path}")
        ...
        if label_video:
            ...
            labelled_vid = deeplabcut.create_labeled_video(config, [vpath], save_frames=True)
