...
from looming_spots.preprocess.photodiode import (
    find_pd_threshold_crossings,
    get_nearest_clock_up,
)
...
def get_loom_idx_on_probe(directory, path_to_traces, upsample_factor=3):
    ...
    for pd_crossing in raw_pd_ups:
        nearest_clock_up_idx, distance_from_clock_up = get_nearest_clock_up(
            pd_crossing, clock_ups_pd
        )
        probe_nth_clock_up_sample = clock_ups_probe[nearest_clock_up_idx]
        new_stimulus_idx = (
            probe_nth_clock_up_sample
            + distance_from_clock_up * upsample_factor
        )
        print(
            probe_nth_clock_up_sample, distance_from_clock_up * upsample_factor
        )
        stimulus_onsets_on_probe.append(
            new_stimulus_idx - 5 * 30000
        )  # FIXME: shifts region to include baseline
...
def get_clock_from_traces(path_to_traces, n_chan=385):
    ...
    if data.shape[0] % n_chan != 0:
        raise "data of length {} cannot be reshaped into {} channels".format(
            data.shape[0], n_chan
        )
