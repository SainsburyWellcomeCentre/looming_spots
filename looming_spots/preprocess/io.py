...
from looming_spots.db.constants import RAW_DATA_DIRECTORY, PROCESSED_DATA_DIRECTORY, \
    AUDITORY_STIMULUS_CHANNEL_ADDED_DATE, OLD_RAW_DIRECTORY
...
def sync_raw_and_processed_data(raw_directory=RAW_DATA_DIRECTORY, processed_directory=PROCESSED_DATA_DIRECTORY):
    cmd = "rsync -tvr --chmod=D2775,F664 --exclude='*.avi' --exclude='*.imec*' --exclude='.mp4' {}/* {}".format(raw_directory, processed_directory)
    subprocess.call(cmd, shell=True)


def sync_raw_spine_and_winstor(raw_directory=OLD_RAW_DIRECTORY,
                               processed_directory=RAW_DATA_DIRECTORY):
    cmd = "rsync -tvr --chmod=D2775,F664 {}/* {}".format(raw_directory, processed_directory)
    subprocess.call(cmd, shell=True)
...
def auto_fix_ai(directory, n_samples_to_replace=500, screen_off_threshold=0.02 ):
    ai = load_pd_on_clock_ups(directory)
    screen_off_locs = np.where(ai < screen_off_threshold)[0]  # TODO: remove hard var
...
def get_all_tracks(raw_directory=RAW_DATA_DIRECTORY, dry=False):
    ...
    for track_path in track_paths:
        ...
        new_path = '{}/{}/{}/{}'.format(PROCESSED_DATA_DIRECTORY, mouse_id, date, fname)
