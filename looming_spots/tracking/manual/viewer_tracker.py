...
class Viewer(object):
    ...
    def on_click(self, event):
        if event.button == 1:
            self.frame_idx = min(self.frame_idx + self.click_step_size, self.video.shape[0] - 1)
            self.update()
        if event.button == 2:
    ...
    def on_scroll(self, event):
        if event.button == 'up':
            self.frame_idx = min(self.frame_idx + self.scroll_step_size, self.video.shape[0] - 1)
...
class TrackViewer(object):
    ...
    def video(self):
        ...
        return np.array(video_processing.get_frames(vid_path, self.track_errors))
    ...
    def add_track(self):
        ...
        self.line_obj, = self.ax.plot(self.tracks[0][:idx], self.tracks[1][:idx])
    ...
    def save(self):
        path = os.path.join(self.track_path, 'tracks.csv')  # FIXME: when there is no tracks.csv file
    ...
    def create_tracks_df(self, path):
        d = {'x_position': self.tracks[0],
             'y_position': self.tracks[1]}
