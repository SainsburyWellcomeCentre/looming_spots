...
class LoomTrial(object):
    def __init__(self, session, directory, sample_number, trial_type, stimulus_type,
                 trial_video_fname='{}{}.h264', contrast=None):
        ...
        self.video_name = trial_video_fname.format(self.stimulus_type, self.stimulus_number())
        self.video_path = os.path.join(self.directory, self.video_name)
        self.folder = os.path.join(self.directory, '{}{}'.format(self.stimulus_type, self.stimulus_number()))
    ...
    def auditory_number(self):
        if self.sample_number in self.session.auditory_idx:
            return int(np.where(self.session.auditory_idx == self.sample_number)[0][0])
    ...
    def fully_sampled_delta_f(self):
        start = self.raw_start() - int(200*10000/FRAME_RATE)  # TODO: derive from raw data instead for precision
        end = start + int(400*10000/FRAME_RATE)
        df = self.session.fully_sampled_delta_f[0][start:end]

        med_range = [int(100*10000/FRAME_RATE), int(190*10000/FRAME_RATE)]  #150, 200
    ...
    def normalised_x_speed(self):  # TODO: extract implementation to tracks
        normalised_x_speed = np.concatenate([[np.nan], np.diff(self.normalised_x_track)])
    ...
    def time_to_reach_home(self):
        return escape_classifiers.time_to_reach_home(self.normalised_x_track, self.context)
    ...
    def extract_track(self, overwrite=True):
        ...
        if self.get_reference_frame() is None:
            ...
            Viewer(self.directory, video_fname='{}{}.h264'.format(self.stimulus_type, self.stimulus_number()), trial_type=self.trial_type)
    ...
    def detect_events(self):
        return self.detect_events_scipy() #detect_events(self.delta_f(), bsl_start=0, bsl_end=200)

    def detect_events_scipy(self):
        df = self.delta_f() - apply_butterworth_lowpass_filter(self.delta_f(), 0.1, 30, order=8)
    ...
    def events(self):
        ...
        all_events = []
        for (s, peak_loc, half_rise, amplitude) in zip(starts, peak_locs, half_rise, amplitudes):
            e = events.Event(s, peak_loc, half_rise, amplitude, self.delta_f(), sampling_interval=1)
            all_events.append(e)
    ...
    def integral(self, end_sample=150):
        ...
        integral = [np.trapz(df[start:start + x], dx=1 / SAMPLING) for x in range(end_sample)]
    ...
    def integral_downsampled(self):
        ...
        mask = np.full(len(df), np.nan)
        integral = [np.trapz(df[LOOMING_STIMULUS_ONSET:LOOMING_STIMULUS_ONSET + x], dx=1 / SAMPLING) for x in range(end_sample)]

        mask[LOOMING_STIMULUS_ONSET:(LOOMING_STIMULUS_ONSET + end_sample)] = integral
        return mask
    ...
    def plot_delta_f_with_track(self, color, scale_factor=10):
        plt.plot(self.delta_f_with_pre_stimulus_events_removed() * scale_factor, color=color, linestyle='--')
        plt.plot(self.normalised_x_track, color=color)
        self.plot_stimulus()
    ...
    def plot_stimulus(self):
        ax = plt.gca()
        if self.stimulus_type == 'auditory':
            patch = patches.Rectangle((200, -0.2), 90, 1.3, alpha=0.1, color='r')
            ax.add_patch(patch)
        else:
...
class VisualStimulusTrial(LoomTrial):
    def __init__(self, session, directory, sample_number, trial_type, stimulus_type='loom'):
        super().__init__(session, directory, sample_number, trial_type, stimulus_type, trial_video_fname='{}{}.h264')

    def make_loom_superimposed_video(self, width=640, height=320, origin=(0, 40)):
...
class AuditoryStimulusTrial(LoomTrial):
    def __init__(self, session, directory, sample_number, trial_type, stimulus_type='auditory'):
        super().__init__(session, directory, sample_number, trial_type, stimulus_type)

    def make_stimulus_superimposed_video(self, width=640, height=250, origin=(0, 40)):
