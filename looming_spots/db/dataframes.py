...
for context in ['A', 'A2', 'A9']:
    for mid in experimental_log.get_combination(include=['post_test', 'habituation'], exclude=['pre_test'],
                                                matching_dict_post_test={'test_type': '== post_test',
                                                                         'context': '== {}'.format(context),
                                                                         'surgery': '== FALSE',
                                                                         'drug_given/additional_param': '== FALSE',
                                                                         'stimulus': '== looming', 'line': '== wt'},
        ...
        try:
            ...
            mouse_df['time_since_habituation'] = mtg.get_trials_of_type('post_test')[0].days_since_last_session_type()
            mouse_df['days_since_habituation'] = [mtg.get_trials_of_type('post_test')[
                                                      0].days_since_last_session_type().days] * 3
...
mouse_ids = experimental_log.get_mouse_ids_with_test_combination(log, ['pre_test'], [])
df = experimental_log.get_subset_df_from_mouse_ids(log, mouse_ids)
for context in ['A', 'A2', 'A9']:
    ids=experimental_log.get_mouse_ids_from_query(df, filter_dict={'test_type': '== pre_test', 'context': '== A9',
                                                  'surgery': '== FALSE',
                                                  'drug_given/additional_param': '== FALSE',
                                                  'stimulus': '== looming', 'line': '== wt'})
    for mid in ids:
